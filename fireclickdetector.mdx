...
The `event{:luau}` argument names which signal on the detector fires. Pass one of `"MouseClick"{:luau}`, `"RightMouseClick"{:luau}`, `"MouseHoverEnter"{:luau}`, or `"MouseHoverLeave"{:luau}`. When omitted, it defaults to `"MouseClick"{:luau}`.
...
| `distance` | `number?` | Simulated activation distance. Defaults to `0{:luau}`. |
| `event` | `string?` | One of the four valid event names. Defaults to `"MouseClick"{:luau}`. |
...
In a tycoon, every upgrade has a buy button you walk up to and click. Fire `MouseClick{:luau}` on all of them and buy the whole tycoon from spawn.
```
