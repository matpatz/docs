...
Pass the function whose frame you want hidden. The optional `hidden{:luau}` flag picks the direction: `true{:luau}` keeps that call out of what `debug.traceback(){:luau}` returns, `false{:luau}` lets it back in.

The code still runs. Nothing about how the function behaves changes, only whether a traceback dump lists it.

Most commonly used to keep an executor function out of an anti-cheat's sight. Even after `newcclosure{:luau}` wraps a script function, its Luau name can still show up in `debug.traceback(){:luau}`. A game that scans the traceback for known names can then flag the user. Hiding the call makes the scan come up empty.
...
See whether your own executor leaks an injected closure's name into `debug.traceback(){:luau}`. Hide the frame so a traceback name-scan can't spot it.
