...
Usually, you can only **assign** a callback like `OnInvoke{:luau}`, but reading it back throws an error. <br /> `getcallbackvalue{:luau}` gets the function you assigned.
...
`function?` - the function assigned to the callback, or `nil{:luau}` if none was assigned.
