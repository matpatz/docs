...
With one argument, this sets `SimulationRadius{:luau}` and also sets `MaximumSimulationRadius{:luau}` to the same value. Pass `max_radius{:luau}` to set the maximum to a different value.
