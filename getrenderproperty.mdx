...
This is the function form of just indexing the property (`drawing.Radius{:luau}`). Use whichever reads cleaner in your code.
...
`any{:luau}` - the property's current value. The type depends on the property (a `Vector2{:luau}` for `Position{:luau}`, a number for `Radius{:luau}`, and so on).
