...
Most commonly used to find per-frame render hooks - things like an ESP or aimbot loop that a script attached to `RenderStepped{:luau}`.

## Returns

`{any}{:luau}` - a table of the callbacks and connections wired to `RenderStepped{:luau}` at the moment of the call.
