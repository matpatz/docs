...
The drawing is not part of the Roblox UI, so game scripts can't see it. Set its properties (like `Position{:luau}`, `Color{:luau}`, `Visible{:luau}`) to control how it looks, and call `:Remove(){:luau}` when you're done with it.
...
`DrawingObject{:luau}` - the new drawing. Every type shares the base properties (`Visible{:luau}`, `ZIndex{:luau}`, `Transparency{:luau}`, `Color{:luau}`) plus the ones specific to its type.
