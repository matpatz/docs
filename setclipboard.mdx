...
The clipboard is OS-visible, so a paste anywhere outside Roblox shows what got written. The function does not enforce a type on `value{:luau}` - a `string{:luau}` lands in the clipboard raw, while a `number{:luau}`, `table{:luau}`, or `nil{:luau}` value is coerced and prefixed with a carriage return byte (`\r{:luau}`).
