...
Most commonly used to run a script hub fetched over HTTP in one line: `loadstring(game:HttpGet(url))(){:luau}`.
...
|---------|--------------|---------------|
| Source compiled cleanly | callable function | `nil{:luau}` |
| Syntax / compile error | `nil{:luau}` | error message |

## Example

Fetch a script hub over HTTP and run it through `loadstring(){:luau}`, guarding the compile so a syntax error in the fetched source warns you instead of failing silently:
```
