...
`value{:luau}` has to match the type already in the slot. There is no coercion.
...
A local starts `false{:luau}` in one frame. A deeper call overwrites that frame's first slot, so the frame returns `true{:luau}` instead.
