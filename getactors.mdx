...
The table is a plain array, so `getactors()[1]{:luau}` is the first actor. Hand one of them to `run_on_actor{:luau}` to run code on that thread.

## Returns

`{Actor}{:luau}` - an array of every [`Actor`](https://create.roblox.com/docs/reference/engine/classes/Actor) instance currently in the game. The array is empty when the game has no actors.
