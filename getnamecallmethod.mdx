...
Most commonly paired with `hookmetamethod{:luau}` to read the method name and decide whether to block, log, or redirect that specific call.

## Returns

`string?{:luau}` - the method name as a string when called from inside a `__namecall{:luau}` metamethod, otherwise `nil{:luau}`.

## Example


Pick any Roblox game, where a LocalScript (or a ModuleScript) triggers a `:Kick(){:luau}` on the `LocalPlayer{:luau}` - detect the `Kick{:luau}` namecall, and silence it before it executes.
