...
`debug.getinfo{:luau}` accepts either a function or a number. Hand it a function and the returned table describes that function. Hand it a number and it is read as a stack level - `0{:luau}` is the call you are in right now, `1{:luau}` is the caller above it, and so on - and the returned table describes whichever frame is running at that level.

Most commonly used while reversing a game script: you find an unknown closure with `getgc{:luau}` and call `debug.getinfo{:luau}` on it to learn what it is, where it came from, and how many arguments it takes before you decide what to do with it.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `level` | `number` | The stack level to inspect. `0{:luau}` is the current frame. |
...
| `short_src` | `string` | Short, printable version of `source{:luau}`. |
| `func` | `function` | The function itself. |
| `what` | `string` | `"Lua"{:luau}` for a Luau function, `"[C]"{:luau}` for a C function. |
| `currentline` | `number` | The line currently running. `-1{:luau}` when no line information is available. |
...
| `numparams` | `number` | Number of declared parameters. Always `0{:luau}` for C functions. |
| `is_vararg` | `number` | `1{:luau}` if the function takes a <abbr title="A function that takes any number of extra arguments - defined as `...` in Luau.">variadic argument</abbr>, `0{:luau}` if it does not. |
...
While reversing a script in a Roblox game, sweep `getgc{:luau}` for an unknown 2-arg Luau closure and identify it before you decide to hook or read it.
