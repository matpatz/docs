...
Most commonly used to tell a real Roblox Instance apart from a Luau table pretending to be one - real Instance methods like `Destroy{:luau}` are always C closures, so a Luau result means the object is a fake.
```
