...
An [Actor](https://create.roblox.com/docs/reference/engine/classes/Actor) is a Roblox object that can run code on its own, separate from your script. `run_on_actor{:luau}` takes that actor and a string of Luau code, and runs the code on the actor. The trailing `...{:luau}` values are passed in as arguments.
...
The actor runs in a separate Lua state, so `shared{:luau}` and `_G{:luau}` <span title="While untested with getgenv(), it's still not recommended." style="text-decoration:underline dotted;text-underline-offset:0.2em;cursor:help;">don't cross between it and the calling script</span> - each side has its own. Pass any value the code needs through the `...{:luau}` varargs instead.
...
The actor runs in a separate Lua state, so `shared` and `_G` <span title="While untested with getgenv(), it's still not recommended." style="text-decoration:underline dotted;text-underline-offset:0.2em;cursor:help;">don't cross between it and the calling script</span> - each side has its own. Pass any value the code needs through the `...` varargs instead.
