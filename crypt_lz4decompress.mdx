...
Input must be LZ4 Block format - see the page entry `crypt.lz4compress{:luau}` to learn more.<br />  `expectedSize{:luau}` is the length of the original plaintext in bytes, *not* the length of the compressed input - you have to track and pass it yourself.

There is no integrity check - if the bytes are damaged or `expectedSize{:luau}` is wrong, <br />the function still returns a string of that length without raising an error.
...
`string` - the decompressed bytes, padded or truncated to exactly `expectedSize{:luau}` characters. <br />When `expectedSize{:luau}` matches the original length, the result is the full plaintext.
