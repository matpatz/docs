...
The result mixes two groups in one table: Instances reachable through `game{:luau}` (`workspace{:luau}`, `Players{:luau}`, the rest of the visible hierarchy), and Instances that exist in memory but have their `Parent` set to `nil{:luau}`.

Common usage: locate an Instance by name when you don't know its path in the hierarchy.

## Returns

`{Instance}{:luau}` - an array of every Instance the client is currently holding alive. Order is not guaranteed.
